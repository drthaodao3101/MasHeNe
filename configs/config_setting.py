...
class setting_config:
    ...
    label_name=["background","tumor","cyst"]
    ...
    patience=100                                                # Train is stopped if model is not better over patience epoches 
    ...
    datasets = 'MasHeNe_65' 
    if datasets == 'MasHeNe_65':
        data_path = './../Dataset2/'                               # Path to MasHeNe dataset
    else:
        raise Exception('datasets in not right!')

    #Get all slice which include no ground truth or not
    filter_no_object_train=True
    filter_no_object_val=True
    filter_no_object_test=True
    ...
    local_rank = -1
    num_workers = 8 
    #seed = 42                          #Seed is set by experiment number, see detail at train.py line 55
    world_size = None
    rank = None
    amp = False
    gpu_id = '1'                                            
    ...
    train_transformer = transforms.Compose([
        ...
        myRandomScale(scale_range=(0.5, 2.0), p=0.75),
        myRandomCrop((input_size_h, input_size_w)),
